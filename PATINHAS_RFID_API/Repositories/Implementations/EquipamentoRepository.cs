...
public class EquipamentoRepository : IEquipamentoRepository
{
    ...
    public async Task<EquipamentoModel?> GetByIdentificador(string identificador)
    {
        var equipamento = await SiagAPI.GetEquipamentoByIdentificadorAsync(identificador);
        ...
        return FormatarOutput(equipamento);
    }
    ...
    {
        ValidarCampos(equipamento, true);

        await SiagAPI.AtualizarEquipamentoAsync(equipamento);
    }
    ...
    private static void ValidarCampos(EquipamentoModel equipamento, bool emEdicao = false)
    {
    ...
    private static EquipamentoModel FormatarOutput(EquipamentoModel output)
    {
        output.EquipamentoModelo = new()
        {
            Codigo = output.IdEquipamentoModelo,
        };

        output.SetorTrabalho = new()
        {
            IdSetorTrabalho = output.IdSetorTrabalho ?? 0
        };

        output.Operador = new()
        {
            IdOperador = output.IdOperador
        };
